...
COPY tourism-coupon-service/src tourism-coupon-service/src

RUN --mount=type=cache,target=/root/.m2 \
    chmod +x mvnw \
    && ./mvnw -B -pl tourism-coupon-service -am package -DskipTests
...
COPY --from=build /workspace/tourism-coupon-service/target/tourism-coupon-service-1.0-SNAPSHOT.jar app.jar
EXPOSE 8084
