...
COPY tourism-user-service/src tourism-user-service/src

RUN --mount=type=cache,target=/root/.m2 \
    chmod +x mvnw \
    && ./mvnw -B -pl tourism-user-service -am package -DskipTests
...
COPY --from=build /workspace/tourism-user-service/target/tourism-user-service-1.0-SNAPSHOT.jar app.jar
